...
//	private JComboBox cmbUsertype= new JComboBox(new String[] { "管理员", "借阅员"});
	public FrmFoodManager_AddFood(JDialog f, String s, boolean b, Map<String, BeanFoodType> ftMap) {
		super(f, s, b);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.btnCancel) {
			...
			if (Double.parseDouble(price) <= 0) {
				JOptionPane.showMessageDialog(null, "数量必须大于0", "错误", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if("".equals(filepath)||(filepath)==null) {
				JOptionPane.showMessageDialog(null, "您还没有上传食材图片\n您之后可以在_修改_中上传图片","提示",JOptionPane.INFORMATION_MESSAGE); 
			}
			try {
				KitchenAssistantUtil.foodManager.add(rt, name, Double.parseDouble(price), Double.parseDouble(count),
						description, specifications, filepath);
				this.setVisible(false);
